...
for key in ['hin_done', 'hin_map', 'hin_csv', 'hin_count', 'hin_edges', 'ksi', 'lat_col', 'lon_col']:
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        if generate:
            ...
            st.write(f'KSI crashes for selected mode: {len(ksi):,}')
            ...
            with st.spinner('Downloading road network...'):
                try:
                    ...
                    st.error(f'Could not download road network for {city}. Try a smaller area. Error: {str(e)}')
                    st.stop()
